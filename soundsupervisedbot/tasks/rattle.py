...
RattlePrimitive = namedtuple('RattlePrimitive', ['elbow_vel', 'acc', 'n'])

class RattleTask(Task):
    def __init__(self, cfg_file = '/home/abitha/projects/sound-supervised-bot/soundsupervisedbot/configs/rattle.yaml', **kwargs) -> None:
    ...
    def sample_primitive(self):
        ...
        return RattlePrimitive(elbow_vel, acc, n)
    
    def _torchify_primitive(self, primitive: RattlePrimitive):
        return torch.tensor([primitive.elbow_vel, primitive.acc, primitive.n])
    
    def _detorchify(self, t_params):
        return RattlePrimitive(t_params[0], t_params[1], int(round_fn(t_params[2])))
    ...
    def _generate_trajectory(self, primitive: RattlePrimitive):
